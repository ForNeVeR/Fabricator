...
function Update-MarkdownFile($relativePath) {
    $file = Resolve-Path "$RepoRoot/$relativePath"

    $oldContent = [IO.File]::ReadAllText($file)
    $newContent = $oldContent -replace '#r "nuget: FVNever\.Fabricator\.(.*?), ([\d.]+)"',
        "#r `"nuget: FVNever.Fabricator.`$1, $NewVersion`""
    [IO.File]::WriteAllText($file, $newContent)
    Write-Output "Updated file `"$file`"."
}

Update-PropsFile 'Directory.Build.props' 'Version'
Update-MarkdownFile 'README.md'
